...
import com.example.pp.util.ViewUtil;
...
public class LoginMainActivity extends AppCompatActivity implements RadioGroup.OnCheckedChangeListener, View.OnClickListener {
    ...
    public void onClick(View v) {
        ...
        if (et_phone.length() < 11){
            Toast.makeText(this, "输入正确达的手机号", Toast.LENGTH_SHORT).show();
            return;
        }
        switch (v.getId()){
            case R.id.btn_forget:
                ...
                if (rb_password. isChecked()){
                    ...
                    //一下弹出提醒对话框，提示用户记住 六位验证码数字
                    AlertDialog.Builder builder = new AlertDialog.Builder(this);
                    builder.setTitle("请记住验证码");
                    builder.setMessage("手机号"+"本次验证码是"+ mVerifyCode +"，请输入验证码是");
    ...
    //定义一个编辑框编辑器，在输入文本达到指定长度是自动隐藏输入法（可惜没实现）
